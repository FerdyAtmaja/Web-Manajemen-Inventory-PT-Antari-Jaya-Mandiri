			...
			<?php 
				...
				elseif($_GET['alert']==2){
					echo "<div class='alert alert-success alert-dismissable'>
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
							<h4><i class='icon fa fa-check-circle'></i>Sukses!</h4>
							Data barang baru berhasil diubah.
						</div>";
				}
				// jika alert=3
				...
				elseif($_GET['alert']==3){
					echo "<div class='alert alert-success alert-dismissable'>
							<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
							<h4><i class='icon fa fa-check-circle'></i>Sukses!</h4>
							Data produk baru berhasil dihapus.
						</div>";
				}
			 ?>
			 		...
			 		<table id="dataTables1" class="table table-bordered table-striped table-hover">
			 			...
			 			<tbody>
			 				<?php 
			 					...
			 					$query=mysqli_query($mysqli,"SELECT a.kd_barang,b.nama_supplier,a.nama_barang,a.harga_beli,a.harga_jual,a.stok FROM tb_barang a INNER JOIN tb_supplier b ON a.kd_supplier = b.kd_supplier ORDER BY a.kd_barang DESC") or die('Ada kesalahan pada query tampil data pakaian: '.mysqli_error($mysqli));
			 					...
			 					while($data=mysqli_fetch_assoc($query)) {
			 						...
			 						echo "<tr>
			 								...
			 								<td class='center' width='80'>
			 									<div>
			 										<a data-toggle='tooltip' data-placement='top' title='Ubah' style='margin-right:5px' class='btn btn-warning btn-sm' href='?module=form_barang&form=edit&id=$data[kd_barang]'>
			 											<i style='color:#fff' class='glyphicon glyphicon-edit'></i>
			 										</a>";
			 				 ?>
			 				 <a data-toggle="tooltip" data-placement="top" title="Hapus" class="btn btn-danger btn-sm" href="modules/barang/proses.php?act=delete&id=<?php echo $data['kd_barang'];?>" onclick="return confirm('Anda yakin akan menghapus barang <?php echo $data['nama_barang']; ?> ?');">
