<?php 
	...
	// jika user sdh login, maka jalankan perintah utk insert, update dan delete
		...
		if($_GET['act']=='insert') {
			...
			$query = mysqli_query ($mysqli, "INSERT INTO tb_barang(kd_barang,kd_supplier,nama_barang,harga_beli,harga_jual, created_user,updated_user) VALUES('$kd_barang','$kd_supplier','$nama_barang','$harga_beli','$harga_jual','$created_user','$created_user')") or die('Ada kesalahan pada query insert : '.mysqli_error($mysqli));
	...
	elseif ($_GET['act']=='update'){
		if(isset($_POST['simpan'])){
			if(isset($_POST['kd_barang'])){
				...
				$query = mysqli_query($mysqli, "UPDATE tb_barang SET 
	...
	elseif($_GET['act']=='delete'){
		if(isset($_GET['id'])){
			...
			$query = mysqli_query($mysqli, "DELETE FROM tb_barang WHERE kd_barang='$kd_barang'") or die('Ada Kesalahan pada query delete : '.mysqli_error($mysqli));
