<?php 
	...
	// jika user sdh login maka jalankan perintah utk insert ,update  dan delete
			...
			elseif (isset($_GET['kode_barang'])){
				...
				$query_pakaian 	= mysqli_query ($mysqli, "SELECT harga_jual, stok FROM tb_barang WHERE kd_barang = '$kd_barang'");
			...
			elseif (isset($_POST['bayar'])){
				...
				$query_keranjang 	= mysqli_query ($mysqli, "SELECT * FROM keranjang_keluar k INNER JOIN tb_barang p ON p.kd_barang = k.kd_barang  WHERE kd_transaksi = '$kd_transaksi'");
				...
				while ($dt = mysqli_fetch_array($query_keranjang)) {
					...
					// update stok pakaian	
					$update_pakaian = mysqli_query($mysqli, "UPDATE tb_barang SET stok = '".$stok."' WHERE kd_barang = '".$dt['kd_barang']."' ");
			...
			elseif (isset($_POST['detail'])) {
				...
				$query_detail_keluar = mysqli_query ($mysqli, "SELECT p.nama_barang, dk.kd_transaksi, dk.jumlah_keluar, dk.harga, dk.sub_total FROM detail_keluar dk INNER JOIN tb_barang p ON p.kd_barang = dk.kd_barang WHERE dk.kd_transaksi = '$kd_transaksi'");
				...
				while ($dt_detail_keluar = mysqli_fetch_array($query_detail_keluar)) {
					...
					$temp[$i]['sub_total'] = number_format($dt_detail_keluar['sub_total'], 0,',','.');
