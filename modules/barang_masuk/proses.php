<?php 
	...
	// jika user sdh login maka jalankan perintah utk insert ,update  dan delete
		...
		if (isset($_POST['kd_transaksi'])) {
			...
			$query_pakaian 	= mysqli_query ($mysqli, "SELECT p.kd_supplier, p.harga_beli, p.stok, s.kd_supplier, s.nama_supplier FROM tb_barang p INNER JOIN tb_supplier s ON s.kd_supplier = p.kd_supplier WHERE p.kd_barang = '$kd_barang'");
			...
			$query_keranjang 	= mysqli_query ($mysqli, "SELECT * FROM keranjang_masuk k INNER JOIN tb_barang p ON p.kd_barang = k.kd_barang  WHERE kd_transaksi = '$kd_transaksi'");
			...
			while ($dt = mysqli_fetch_array($query_keranjang)) {
				...
				$update_pakaian = mysqli_query($mysqli, "UPDATE tb_barang SET stok = '".$stok."', harga_beli = '".$dt['harga']."' WHERE kd_barang = '".$dt['kd_barang']."' ");
			...
			$query_detail_masuk = mysqli_query ($mysqli, "SELECT p.nama_barang, dm.kd_transaksi, dm.jumlah_masuk, dm.harga, dm.sub_total FROM detail_masuk dm INNER JOIN tb_barang p ON p.kd_barang = dm.kd_barang WHERE dm.kd_transaksi = '$kd_transaksi'");
			...
			while ($dt_detail_masuk = mysqli_fetch_array($query_detail_masuk)) {
				...
				$temp[$i]['sub_total'] = number_format($dt_detail_masuk['sub_total'], 0,',','.');
